...
--AND i.name = 'IX_Your_Index_Name' --only check a specified index
 order by ius.user_seeks desc
	   ...
	   ,ius.last_user_seek desc
	   ,ius.last_user_scan desc
	   ,ius.last_user_lookup desc
	   ,ius.last_user_update desc

-- Look out for indexes where no. of Seeks,Scans and Lookups operations are 0. It means that they are not getting used at all
-- also need to consider how old the data is
-- Also consider the indexes where there is a significant difference between Seek\Scan vs Updates.
